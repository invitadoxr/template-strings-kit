...
const {
    getStoreDirPathAsync, writeFileAsync, getTemplateStringInfoAsync,
    getCompletionListFromFilePathAsync, completionItemsExpliciter,
    readFileAsync,
...
async function activate(context) {
    ...
    try {
        ...
        vscode.workspace.onDidChangeTextDocument(async (ev) => {
            ...
            const { content, isInside, relativePosition, language, range } = await getTemplateStringInfoAsync(doc, pos, languagePrefixPattern);
            if (!isInside) return null;

            const filePath = language === "html" ? htmlPath : language === "jsx" ? jsxPath : cssPath;
            await writeFileAsync(filePath, content);

            if (language !== "jsx") return null // diagnostics only for jsx
            await createJsxDiagnosticFromContentInRange( diagColl, doc, content, range)
        })

        // autocomplete
        const disposableAutoComplete = vscode.languages.registerCompletionItemProvider(
            { scheme: 'file', language: 'javascript' },
            {
                async provideCompletionItems(document, position, token, context) {
                    const { content, isInside, relativePosition, language, range } = await getTemplateStringInfoAsync(document, position, languagePrefixPattern);
                    if (!isInside) return null;
                    const filePath = language === "html" ? htmlPath : language === "jsx" ? jsxPath : cssPath;

                    const completionList = await getCompletionListFromFilePathAsync(filePath, content, relativePosition);
